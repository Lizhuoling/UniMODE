...
class VoxelPoolingTrain(Function):
    ...
    def forward(ctx, geom_xyz: torch.Tensor, input_features: torch.Tensor,
        ...
        geom_xyz = geom_xyz.reshape(geom_xyz.shape[0], -1, geom_xyz.shape[-1])  # Left shape: (B, N, 3)
        input_features = input_features.reshape((geom_xyz.shape[0], -1, input_features.shape[-1]))  # Left shape: (B, N, C)
        assert geom_xyz.shape[1] == input_features.shape[1]
        batch_size = input_features.shape[0]
        num_points = input_features.shape[1]
        num_channels = input_features.shape[2]
        output_features = input_features.new_zeros(batch_size, voxel_num[2],
