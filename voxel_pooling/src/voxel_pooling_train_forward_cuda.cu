...
__global__ void voxel_pooling_train_forward_kernel(
  ...
  if (thread_sample_idx < total_samples) {
    ...
    if ((sample_x >= 0 && sample_x < num_voxel_x) &&
      ...
      pos_memo[thread_sample_idx * 3 + 0] = thread_sample_idx / num_points; // bs
      pos_memo[thread_sample_idx * 3 + 1] = sample_z; // bev_z
      pos_memo[thread_sample_idx * 3 + 2] = sample_x; // bev_x
    }
  }
  ...
  for (int i = tidy;
    ...
    for (int j = tidx; j < num_channels; j += THREADS_BLOCK_X) {
      atomicAdd(&output_features[(batch_idx * num_voxel_z * num_voxel_x + // num_voxel_y should be 1
                                  sample_z * num_voxel_x + sample_x) *
                                     num_channels +
                                 j],
