...
class VisionTransformer(BaseModule):
    ...
    def forward(self, x, return_patch_tokens=True):
        x = self.forward_features(x, return_patch_tokens=return_patch_tokens)   # (B, L, C)
        #x = self.head(x)
        return x
...
def remove_rope(checkpoint):
    for key in list(checkpoint['module'].keys()):
        if 'rope' in key:
            del checkpoint['module'][key]
        if 'pos_embed' in key:
            del checkpoint['module'][key]
