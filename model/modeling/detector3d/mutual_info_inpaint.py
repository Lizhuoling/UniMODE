...
class MutualInfoInpainter(BaseModule):
    ...
    def forward(self, cam_bev_feat, point_bev_feat, cam_valid_mask, pts_valid_mask):
        '''
        Input:
        ...
        if self.pts_paint_cam:
            cam_inpaint_loss = self.inpaint_loss_weight * F.l1_loss(cam_bev_feat * pts_valid_mask.unsqueeze(1), (point_bev_feat * pts_valid_mask.unsqueeze(1)).detach(), reduction = 'mean')
        ...
        if self.cam_paint_pts:
            ...
            point_inpaint_loss = self.inpaint_loss_weight * F.l1_loss(point_bev_feat * supervise_point_mask.unsqueeze(1), (cam_bev_feat * supervise_point_mask.unsqueeze(1)).detach(), reduction = 'mean')
